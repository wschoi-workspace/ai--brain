"""
메종조 SNS 분석 파이프라인 v1.0
크롤링 → Sincerity Filter → 2-Layer → Linguistic Layer → 리포트

사용법:
  python maisonjo_pipeline.py [--days 90] [--skip-crawl] [--skip-linguistic]

전체 파이프라인:
  1. maisonjo_crawl.py 실행 (데이터 수집 + 2-Layer 분석)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='메종조 SNS 분석 파이프라인')
    ...
    print("=" * 60)
    print(f"메종조 SNS 분석 파이프라인 v1.0 — {today}")
    print("=" * 60)
    ...
    if not args.skip_crawl:
        print("\n[Phase 1] 데이터 크롤링")
        crawl_script = SCRIPT_DIR / 'maisonjo_crawl.py'
