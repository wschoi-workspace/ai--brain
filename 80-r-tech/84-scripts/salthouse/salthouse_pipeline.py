"""
소금집 SNS 분석 파이프라인 v1.0
크롤링 → Sincerity Filter → 2-Layer → Linguistic Layer → 리포트

사용법:
  python salthouse_pipeline.py [--days 90] [--skip-crawl] [--skip-linguistic]

전체 파이프라인:
  1. salthouse_crawl.py 실행 (데이터 수집 + 2-Layer 분석)
...
CONFIG_PATH = PROJECT_ROOT / '10-projects' / '29-salthouse-sns' / 'config.json'
...
def main():
    ...
    parser = argparse.ArgumentParser(description='소금집 SNS 분석 파이프라인')
    ...
    print("=" * 60)
    print(f"소금집 SNS 분석 파이프라인 v1.0 — {today}")
    print("=" * 60)
    ...
    if not args.skip_crawl:
        print("\n[Phase 1] 데이터 크롤링")
        crawl_script = SCRIPT_DIR / 'salthouse_crawl.py'
