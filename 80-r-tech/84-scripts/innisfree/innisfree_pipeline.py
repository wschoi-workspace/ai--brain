"""
이니스프리 SNS 분석 파이프라인 v1.0
크롤링 → Sincerity Filter → 2-Layer → Linguistic Layer → 리포트

사용법:
  python innisfree_pipeline.py [--days 90] [--skip-crawl] [--skip-linguistic]

전체 파이프라인:
  1. innisfree_crawl.py 실행 (데이터 수집 + 2-Layer 분석)
...
def analyze_pragmatics(text):
    ...
    conditional_rec = re.findall(r'(?:좋아하\S*|피부\S*|타입\S*)면\s*(?:추천|강추|좋)', text)
    implicit_comp = re.findall(
        r'(?:다른|타)\s*(?:제품|브랜드|화장품|세럼|크림|토너)\S*\s*(?:다르|달리|비해|보다)', text
    )
...
def calculate_eds(discourse, pragmatics, text):
    ...
    # 화장품 카테고리: 용량/기간/가격 수치 + 구매 채널
    details = re.findall(
        r'\d+\s*(?:원|개|ml|mL|호|주|주일|개월|일째|일간|번|퍼|%)'
        r'|올리브영|쿠팡|공홈|면세점|백화점|약국|마트|네이버쇼핑|라이브',
        text
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(description='이니스프리 SNS 분석 파이프라인')
    ...
    print("=" * 60)
    print(f"이니스프리 SNS 분석 파이프라인 v1.0 — {today}")
    print("=" * 60)
    ...
    if not args.skip_crawl:
        print("\n[Phase 1] 데이터 크롤링")
        crawl_script = SCRIPT_DIR / 'innisfree_crawl.py'
