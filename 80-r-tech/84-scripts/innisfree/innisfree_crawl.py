...
사용법:
  python innisfree_crawl.py [--days 90] [--output data/2026-06-07]
...
CONFIG_PATH = PROJECT_ROOT / '10-projects' / '40-innisfree-sns' / 'config.json'
...
def crawl_naver_blogs(queries, client_id, client_secret, config, days_back=None):
    """전체 키워드에 대해 네이버 블로그 크롤링

    days_back이 주어지면 sort='date' 페이징 시 기간을 벗어난 페이지에서 조기 종료.
    (최근 N일 분석에서 불필요한 API 호출을 줄임)
    """
    ...
    sort_modes = crawl_config.get('sort_modes', ['sim', 'date'])
    ...
    for query in queries:
        for sort_mode in sort_modes:
            ...
            for start in range(1, max_start + 1, display):
                ...
                # date 정렬: 페이지 전체가 기간 밖이면 이후 페이지도 모두 밖
                if cutoff and sort_mode == 'date':
                    if all(it.get('postdate', '') < cutoff for it in items):
                        break
...
def crawl_full_content(posts, delay=0.3, workers=5):
    """전체 포스트 본문 크롤링 (스레드 병렬)"""
    ...
    with ThreadPoolExecutor(max_workers=workers) as ex:
        futures = [ex.submit(_fetch, (i, p)) for i, p in enumerate(posts)]
        for fut in as_completed(futures):
            idx, content = fut.result()
            posts[idx]['full_content'] = content
            if content and len(content) > 100:
                crawled += 1
            done += 1
            if done % 25 == 0:
                print(f"  본문 크롤링: {done}/{len(posts)}건 (확보: {crawled}건)")

    for p in posts:
        p.setdefault('full_content', '')
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description='이니스프리 SNS 크롤러')
    ...
    print("=" * 60)
    print(f"이니스프리 SNS 크롤러 v1.0 — {today}")
    ...
    raw_posts = crawl_naver_blogs(queries, client_id, client_secret, config, days_back=args.days)
