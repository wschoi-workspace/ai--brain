...
BASE = Path(r'C:\Users\insig\do-better-workspace\80-r-tech\85-analysis-results\hellokitty-jisoo')
...
rql_bars = (rql_row('Q5 서사형 ×2.0', 'Q5_서사형', 'var(--primary)')
            ...
            + rql_row('Q1 간단형 ×0.2', 'Q1_간단형', '#94a3b8'))
...
rql_weighted = sum((1 if k == 'Q3_경험형' else 2.0 if k == 'Q5_서사형' else 1.5 if k == 'Q4_분석형' else 0.5 if k == 'Q2_감상형' else 0.2) * v for k, v in RQL.items())
...
<div class="rawbox">
  <div style="font-size:12px;font-weight:700;color:var(--primary);margin-bottom:6px;">📊 협찬 {SPON_N}건 상위 Auth Raw Data (상위 12건)</div>
  {make_spon_details()}
</div>
...
<div class="concl"><strong>왜 → 뭘 의미 → 차별화:</strong> Q5 서사형 {RQL.get('Q5_서사형',0)}건 + Q4 분석형 {RQL.get('Q4_분석형',0)}건. 이는 <strong>헬로키티 IP + 지수 조합이 블로거의 "긴 글 동기"를 자극</strong>했음을 의미. 의미: 단순 감탄형(Q1~Q2)이 {RQL.get('Q1_간단형',0)+RQL.get('Q2_감상형',0)}건으로 28%에 달해 거품도 공존. 차별화: 기존 도구는 "{TOTAL}건" 단일 숫자, RXR은 <strong>Q5=2.0 가중 적용시 유효 건수를 "{rql_weighted:.0f}건"으로 재계산</strong> 가능하다.</div>
...
def write_summary_sheet(ws):
    ...
    ws.append(['■ 기간별 통계'])
    ws[ws.max_row][0].font = Font(bold=True, size=12, color='6666FF')
    ...
    for c in ws[ws.max_row]:
        c.fill = HDR_FILL
        c.font = HDR_FONT
    weights = {'Q5_서사형': 2.0, 'Q4_분석형': 1.5, 'Q3_경험형': 1.0, 'Q2_감상형': 0.5, 'Q1_간단형': 0.2}
    for k in ['Q5_서사형', 'Q4_분석형', 'Q3_경험형', 'Q2_감상형', 'Q1_간단형']:
        cnt = RQL.get(k, 0)
    ...
    for col, w in zip('ABCDEFG', [32, 12, 12, 10, 10, 12, 12]):
