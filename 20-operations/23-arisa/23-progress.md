...
## 2026-08-30 (6차) — 프로젝트 일정 운영 스코프 필터 (배포 완료)

대표 지시: "운영팀에는 프로젝트 일정에 운영 관련된 내용만 뜨게 옵션(필터링)".

### 판별 — 새 필드를 만들지 않았다
프로젝트 tasks의 **`division`이 이미 팀 이름**을 담고 있었다(실측: 운영팀 77 · 경영 57 ·
사업기획 45 · 공간팀 43 · 기획팀 31 + 영문 Operation 9 · Space 14 · Business 6).
`_OPS_DIVISIONS` = 운영팀·운영·operation(s)·ops·매장·store (`ARISA_OPS_DIVISIONS`로 조정).

### 적용
- `/api/my-work?ops=1` — 내 프로젝트 일정에서 division이 운영 계열인 업무만
- `/api/lead-home?ops=1` — 운영 업무 0건인 프로젝트는 제외 + **진행률도 운영 업무 기준 재계산**
- 둘 다 `ops_hidden`을 함께 반환 → 헤더에 "운영만 ✓ · 4건 숨김"으로 표기(조용히 숨기지 않는다)

### 기본값
`ops_member(uid)` = 운영팀 소속 or 운영팀 리더, **대표 제외**(전사를 봐야 하므로).
→ 윤혜정·양은정·김준호 기본 ON / 최원석·양지혜·전제훈·배성원 기본 OFF.
사람이 끄고 켠 값은 `localStorage.ops_only`에 기억(기본값보다 우선).
옛 캐시 세션엔 `ops_member`가 없어 `/api/me` 응답으로 다시 잡고 바뀌면 재렌더.

### 검증
| 항목 | 결과 |
|---|---|
| ops_member 7명 판정 | 운영 3명만 True · 대표 False |
| 맥미니 실데이터 | 윤혜정 **13건 → 9건**(현대차·관광오픈이노베이션·중기팝업·월드모드홀딩스 제외 — 넷 다 운영 division 0) |
| 진행률 재계산 | CJ×KBO 74건 → 25건 · 롱블랙 22 → 12 · 아랑재 30 → 9 |
| 토글 | 3↔5 왕복 · 새로고침 후 선택 유지(ls=0/1) |
| 기타 | 대표·기획팀 기본 '전체 보는 중' · JS 에러 0 · 미정의 호출 스캔 0 · arisa-os.com 정상 |

### 남은 것
- 양은정·김준호는 **프로젝트 tasks에 owner로 배정된 게 0건**이라 이 필터로 보이는 변화가 없다.
  매장 업무가 프로젝트 task로 등록되기 시작하면 그때부터 효과가 난다(분장은 별개로 잘 흐르고 있음)

## 2026-08-30 (5차) — 매장 일일보고 AI 정리(매출 중심) + 매출 엑셀 업로드 (배포 완료)

대표 지시 2건: ①봇에 올라온 일일보고를 자동 정리해 매장에 반영 ②월간·주간 매출 엑셀 업로드 섹션.

### 먼저 확인한 것 — 배선은 이미 돌고 있었다
- 바스켓 봇 → 일일보고 시트: **작동 중**(8/25까지, store_id도 basket/arangje 분류 정상)
- 일일보고 시트 → 매장 화면: **작동 중**(3차에서 만든 `/api/stores`)
- 진짜 문제는 **정리** — 셀 원문을 그대로 붙여 '진행'·'완료' 한 단어까지 이슈로 떴다

### ① 정리 = 규칙 + AI(매출 중심)
- `_store_clean()` — 무의미 토막(`_STORE_NOISE`)·중복 제거, 줄바꿈/불릿 분해
- `_store_digest()` — 매장별 `{headline, sales, issues, voc}`를 LLM이 생성.
  프롬프트에 **"매출을 중심에 둔다"**를 못박고, 금액은 원문 표기 그대로·없는 사실 금지
- **날짜별 파일 캐시**(`_data/store-digest/YYYY-MM-DD.json`). 캐시가 없으면 규칙 결과를
  즉시 주고 **백그라운드 스레드로 생성** → 화면 진입이 LLM을 기다리지 않는다
- **원문은 카드 안 `<details>`에 그대로** — AI 결과를 눈으로 대조할 수 있게(검증 가능한 것만 맡긴다)
- 키 없거나 실패 시 규칙 정리로 자연 폴백

### ② 매출 자료 업로드 — 자유 서식·원본 보관
- 우열 3단으로: `💰 매출` / `📊 매출 자료`(신설) / `✅ 체크사항`
- `POST /api/store-sales/upload`(store_access) · `/delete`(**대표만** — 기록이 조용히 사라지면 안 된다) ·
  `GET /api/store-sales/file`(원본 내려받기, **경로 이탈 차단**: 인덱스에 있는 id + 보관폴더 하위만)
- 저장 `_data/store-sales/<store>/<kind>/<타임스탬프>__<원본명>` + `index.json`
- `_sales_scan()` — 헤더 추정(숫자가 가장 적은 상위 5행 중 하나), **절반 이상이 숫자인 컬럼만**
  금액 컬럼으로 간주해 합계·행수·미리보기 추출. 서식이 바뀌어도 업로드 자체는 안 깨진다
- 두 폴더 모두 `.gitignore` 등재(vNext W0-3 런타임 산출물 규칙) — 맥미니에서 `check-ignore`로 확인

### ⚠️ 잡은 함정 — 문법 검사가 못 잡는 종류
리팩터링 중 블록 교체로 **`stSalesHtml`이 통째로 사라졌는데 `node --check`는 통과**했다
(미정의 함수 호출은 문법 오류가 아니다). 화면이 안 뜨는 걸 보고 발견 →
**미정의 호출 스캔**(정의된 함수명 vs 호출된 이름 차집합)을 따로 돌려 확인하는 절차 추가.

### 권한 (대표 확인)
"운영팀과 운영리드만 활성화" 지시 → 글자 그대로면 대표가 빠져 되물음 →
**대표 유지 확정**. 현행 `store_access`(대표+운영팀 리더+운영팀 4명) 그대로, 코드 변경 없음.

### 검증
| 항목 | 결과 |
|---|---|
| AI 요약 실생성 | 노트북 5초 / 맥미니 실환경 생성 확인. **리진 헤드라인이 '일매출 266,500원 기록'** — 매출 중심 작동 |
| 규칙 정리 | 바스켓 8건이 문장 단위로 분해, '진행'·'완료' 단어 제거 확인 |
| 엑셀 업로드 왕복 | 25행 샘플 → **합계 10,558,000 = 샘플 실제값 정확 일치** · 디스크 보관·index.json·시트 2개 인식 |
| 화면 | 패널 5개 · 업로드 폼 노출 · 페이지 스크롤 0 · 가로 오버플로 0 · JS 에러 0 |
| 배포 후 | 맥미니 md5 일치 · openpyxl 3.1.5 · 미로그인 401 · arisa-os.com 신규 CSS 3종 적재 · JS 에러 0 |
| gitignore | store-digest·store-sales 둘 다 `git check-ignore` 매칭 확인 |

### 남은 것
- 매출 세분화(커피·디저트·굿즈·이벤트)는 여전히 **마감보고가 안 올라와서** 비어 있다.
  이제 엑셀 업로드가 생겼으니 매출은 그쪽으로 받는 게 현실적 — 마감보고 필드 확장은 보류
- `_sales_scan` 합계는 **숫자 컬럼 자동 감지 추정값**이다(툴팁에 명시). 실파일 서식을 주시면 전용 파서를 덧붙인다

## 2026-08-30 (4차) — HR·입퇴사 온보딩 탭바 통합, 온보딩 대표 전용 (배포 완료)

대표 지시: "HR이랑 입퇴사 온보딩 제일 윗줄에 통합, 입퇴사 온보딩은 내 전용".

- 탭바에 `입퇴사 온보딩 ↗`(`tab-onboard`) 신설 — HR 포털 탭 옆, **대표만 노출**
- '오늘 할 일' 패널 안 링크 2개 제거. 같은 링크가 두 군데 있었고 패널 첫 화면을 차지했다
- 주소를 화면에 박지 않는다 — `__HR_ONBOARD_URL__` 치환으로 `HR_PORTAL_URL`을 따라간다
  (`__TASK_TYPES__`와 같은 주입 패턴). 쓰임 없어진 `.mw-quick`/`.mw-link` CSS 제거
- ⚠️ 탭 숨김은 화면 정리이지 유일한 방벽이 아니다 — `/onboard/admin/v2`는 미인증 시
  **302 → 포털 루트**로 튕긴다(curl 확인). 접근 통제는 HR 포털이 계속 강제한다

검증: 3계정 실측(대표 탭 8개·온보딩 노출·핸들러 바인딩 / 윤혜정·양지혜 온보딩 없음) ·
패널 내 잔여 링크 0 · 플레이스홀더 치환 로컬·맥미니 양쪽 확인 · JS 에러 0 ·
arisa-os.com 탭 DOM에 `입퇴사 온보딩 ↗` 존재(미로그인이라 숨김 상태)

## 2026-08-30 (3차) — 일정표 개인/업무 분리 + 등록자 라벨 (배포 완료)

대표 지시: "내 개인으로 등록된 일정은 숨기고, 업무로 등록한 건 보이게 라벨 구분".

### ⚠️ 먼저 확인한 것 — 판별 근거가 거의 없었다
실제 캘린더 18건을 뜯어보니 **colorId·visibility·attendees를 한 건도 안 쓰고 있다.**
쓸 수 있는 건 **출처 캘린더**뿐이었다.
- PROJECT RENT 그룹 8건 = 전부 팀원 등록(전제훈·배성원·김가은·양지혜) → 업무 확실
- timehora 개인 1건("다도") → 개인 확실
- **ws.choi 9건 = 업무·개인 혼재** (아랑재 오픈·크래프톤 자문·매출정산 vs 저녁식사·차회 예약)

### 채택 (출처 + 제목 표식 병행)
- `CAL_WORK`(그룹) → `kind=work` · `CAL_PERSONAL`(timehora)에서 온 것 + MY 병합분 → `personal`
- 제목에 `[개인]·(개인)·#개인·[사적]·[private]` → `personal` (`CAL_PERSONAL_RE`)
- 나머지 MY(ws.choi) → `mine`
- 기본은 personal 제외 + 숨긴 건수만 표기, 패널 헤더 **'개인 포함' 토글**로 켠다

### ⚠️ 잡은 함정 — 병합 자동화가 시각을 바꾼다
'다도'가 개인 캘린더 **12:00**인데 ws.choi 병합본은 **09:00**이었다.
(날짜,시각,제목) 키로는 병합본을 개인으로 못 잡아 두 번 나왔다.
→ 개인 판정 키는 `_cal_daykey()` = **(날짜, 제목)**, 중복 제거는 시각까지 쓰되
개인 건만 (날짜,제목)으로 한 번 더 접는다.

### 라벨 — `업무 · 전제훈`
`arisa-employees.json:by_calendar_email` 신설(8건). 근거는 **사내 메일 헤더**와
표기 규칙 `{이니셜}.{성}@project-rent.com`:
`jehun jeon <r.jjehun@gmail.com>`=전제훈 · `배성원 <sungwon.bae2@>` · `jh.yang`=양지혜
(`hj.yoon`=윤혜정 · `dy.kim`=김도영과 동일 규칙으로 교차확인).
- `r.rlarkdms@gmail.com`=김가은은 **두벌식 표기 추론**(rla=김, rkdms=가은)이며 메일 헤더 확인분은 아니다 — 파일 주석에 명시
- 미등록 이메일은 이름 없이 '업무'로만. `admin@project-rent.com`(병합 자동화 계정) 제외

### ⚠️ 지시 범위 밖이지만 같이 막은 누출
**직원·리더 화면 팀 일정표에 대표 개인 캘린더가 그대로 보이고 있었다** —
'저녁식사 (김은영, 이나리). 가토스', '산수화 X 소요상회 차회 예약' 등.
팀 일정표는 팀 일정을 보는 자리다. → `_cal_owner()`로 **MY 캘린더는 주인에게만**,
남에겐 그룹 캘린더만 + 부제에 '팀 일정만'. 남의 개인 **건수도** 알리지 않는다.
되돌리려면 `ARISA_CALENDAR_MINE`을 비운다.

### 검증
| 항목 | 결과 |
|---|---|
| 3계정 실측 | 최원석 17건(업무8+내9)·개인1 숨김·토글O / 윤혜정·양지혜 **업무 8건만**·토글X·**사적 노출 0** |
| 토글 왕복 | 17건 → 18건(개인 '다도' 등장) → 17건 |
| 맥미니 실환경 | owner=최원석·매핑 8건 · 최원석 17건 / 양지혜 8건(mine_shown False) |
| 문법 | ast.parse + 런타임 JS `node --check` OK · JSON 파싱 OK |
| 공개 도메인 | arisa-os.com 200 · `.cal-tag`·`.work`·`.personal` CSS 적재 · JS 에러 0 |

### 캘린더 원본 수정 (대표 승인 후 실행)
ws.choi에 직접 넣은 사적 일정 2건 제목에 `[개인]` 추가. 단발·본인 소유라 안전.
`--dry-run`으로 PATCH URL·본문(summary만)을 먼저 확인하고 실행했다.

| eventId | 변경 |
|---|---|
| `g3mes6hi3cpd01j0v7lvft63dg` | 산수화 X 소요상회 차회 예약 → **[개인]** 산수화 X 소요상회 차회 예약 |
| `t01flh48qrhmjoqjllhd28sf2s` | 저녁식사 (김은영, 이나리). 가토스 → **[개인]** 저녁식사 (김은영, 이나리). 가토스 |

되돌리려면 같은 eventId로 `summary`에서 `[개인] `만 빼면 된다.
검증: 캘린더 재조회로 제목 확인 → 맥미니 실환경에서 **최원석 15건·개인 숨김 3 · 사적 노출 0**,
양지혜 8건. 배지가 이미 '개인'이라 화면 제목에서는 표식을 지워 보여준다(원본은 유지 — 판별 근거).

### 남은 것
- `r.rlarkdms@gmail.com` = 김가은 매핑은 추론 — 틀리면 `by_calendar_email` 한 줄만 고치면 된다
- 앞으로 개인 일정은 제목에 `[개인]`을 붙이거나 timehora 캘린더에 넣으면 자동으로 숨겨진다

## 2026-08-30 (2차) — 원안 2열 패널 레이아웃 전환 · PC 한 화면 고정 (배포 완료)

1차에서 기능은 다 넣었지만 **배치가 원안과 달랐다** — 전부 `max-width:1000px` 한 컬럼 세로 스택.
대표 화면은 블록 8개라 한참 스크롤해야 했고 1440px에서 좌우 400px씩 비었다.

### 원안이 말하는 것 (pptx 좌표 재추출)
- 5장 전부 같은 골격: **좌우 2열(각 5.28in)** · **좌상단 고정 팀일정표** · `"스코롤 통해서 아래로 확장"` **6회**
- 즉 16:9 한 화면에 패널을 다 놓고 넘치면 패널 안에서 굴린다. 행 비율을 `grid-template-rows`에 그대로 넣었다
- 슬라이드1 대표 좌[2.77/1.99] 우[2.31/1.45/0.97] · 2 리더 좌[2.77/1.99] 우[1.13/1.55/1.99] · 3 Staff 우[1.92/**내일할일 1.25**/1.60] · 4 매장 좌[3.35] 우[매출/체크] + 하단 전폭 4열

### 구현 (`bf7c141` → 맥미니 `c7cc741`, 단일 파일)
- `.dash`/`.dash-col`/`.pn`/`.pn-h`/`.pn-b` + `pnl()`·`pnSec()`·`pnBindGo()` 헬퍼
- `#f-mywork`·`#f-stores` `overflow:hidden` — 페이지 스크롤 없음
- **폴백**: `max-width:1199px` 또는 `max-height:699px` → 기존 1열 스택 + 페이지 스크롤
- 대표: 좌[일정표/프로젝트별 이슈=진행상황+Risk+내 프로젝트] 우[오늘 할 일=핵심+Decision+Intervention+결재+내 분장+분장등록(접힘) / 이번 주=승인+지연+미지정+PM클리어 / **🏪 매장 요약(신규)**]
- 리더: 우[업무 분장·필요 항목 / 오늘 할 일 / 이번 주] · Staff: 우[오늘 할 일 / **🌙 내일 할 일(신규)** / 이번 주]
- 매장: 상단 2열[일정표 | 매출+체크사항] + 하단 전폭 매장 4열
- **🌙 내일 할 일은 읽기 전용** — 내일 마감 + 오늘 선언 미완을 클라이언트 산출. `/api/today-plan`이 서버에서 오늘 날짜를 고정해 '내일 선언' 토글은 후속
- 서버 API·권한·데이터 로직 무변경

### ⚠️ 잡은 버그 2건
1. **`#f-stores{display:flex}`가 `.frame{display:none}`을 이겼다** — ID 우선순위. 탭을 바꿔도 매장 화면이 계속 떠서 두 화면이 반반 겹쳐 나왔다. 자동 지표(페이지스크롤 0·패널 수)는 전부 통과했고 **캡처를 눈으로 보고 발견**. → `#f-stores.on`으로 수정
2. **런타임 JS 검사 필요** — `node --check`를 파일 원문에 돌리면 `\n` vs `\\n` 이스케이프 실수를 못 잡는다(UNIFIED_SHELL이 raw string이 아님). 모듈을 import해 **Python이 처리한 뒤의 문자열**을 검사하도록 바꾸니 바로 FAIL이 나왔다. 검사 스크립트를 이 방식으로 고정할 것

### 검증
| 항목 | 결과 |
|---|---|
| 해상도 6종 | 1920·1440·1280 **페이지스크롤 0**·패널 5개·내부스크롤 동작 / 1100·1024·1440×680 **스택 복귀** |
| 4역할 화면 | JS 에러 0 · 가로 오버플로 0 |
| 핸들러 회귀 | 상태버튼 80/80 · ☀️ 8/8 · 수정 8/8 · 배정 11/11 (`.ua-as`는 `.ua-go`가 읽는 설계라 무바인딩이 정상) |
| 바인더 호출 | 3함수 전부 before==after + `pnBindGo`만 추가 |
| 서버 왕복 | ☀️ 오늘 선언 0→1→0 |
| 공개 도메인 | arisa-os.com 200 · 패널 CSS 적재 · JS 에러 0 |

### 산출물
- `arisa-화면미리보기-20260830.html` — 실제 배포 화면 캡처 8장 + 원안 대응 + 검증 기록
- `arisa-개인메인-레이아웃검토-20260830.html` 상단에 후속 반영 노트 추가

### 남은 것
- ⏳ 실계정 로그인 육안 확인 (PIN 없음 — 코드·데이터·게이트·공개 도메인은 검증됨)
- 후속 과제 A~I는 검토 리포트 §5 · 여기에 **'내일 선언' 토글(서버 date 파라미터)** 추가

## 2026-08-30 (1차) — 개인 메인 화면 레이아웃 검토 + 🏪 매장 탭 신설 (배포 완료)

원안 `Dropbox/13-업무자동화/레이아웃1.pptx`(5장: 대표/팀리더/Staff/Basket매장/매장단일뷰) 대 라이브 대조.

### 검토 결과 — 원안 13요소 중
- **구현 6** (오늘 할일 · 업무분장 담당자지정 · 완료승인 제거 · 결제승인 등)
- **부분 4** — 데일리루틴(메일·미팅 없음) / 이번주 진행할 일에 **산출물 축 없음**(시트 G열은 있는데 화면이 안 읽음) / 팀 프로젝트별 **이슈 텍스트 없음**(신호등·%만) / 참여 프로젝트 이슈 없음
- **없음 3** — 구글 팀일정표(연동 0) · 담당자 코멘트 칸 · Staff 내일 할일 · 부가업무/잡무 분류축
- 원안보다 앞선 것: Decision/Intervention/Risk 3분류, 승인 체인 단계, ReportScore, 마감 달력, 입력 완결성, 오늘선언 실행률
- 리포트: `arisa-개인메인-레이아웃검토-20260830.html`

### 구현 ① 팀 일정표 (전 역할 공통)
- `shared/gws.py:calendar_events()` — values_get과 같은 subprocess·재시도·auth분류 패턴
- `/api/calendar` — ws.choi 메인 + PROJECT RENT 그룹 병합, 오늘~+14일, **날짜·시각·제목 중복 제거**(두 캘린더가 병합돼 있어 같은 일정이 두 번 온다)
- 내 업무 · 팀 홈 · 매장 화면 **최상단**에 삽입. **읽기 전용** + "구글 캘린더에서 추가 ↗"
  (쓰기=`events insert`는 권한 경계 설계가 별건 — 같이 넣으면 배포 검증이 흐려진다)

### 구현 ② 🏪 매장 탭
- `store_access(uid)` = 대표 or 운영팀 리더 or 운영팀 소속 (`STORE_TEAMS={"운영팀"}`, 명부 기반 → 신규 입사자 자동 포함)
  → **최원석·윤혜정·양은정·김준호 4명만**. 12명 전수 확인, 기획팀 staff 미포함
- **게이트 2층**: 서버 403(`/api/stores`·`/store-insight/*`) + `/api/me` 응답으로 탭 표시.
  로그인 캐시가 낡아도 매번 서버에 다시 물으므로 명부 변경이 다음 접속에 반영
- 서브탭 일일/주간/월. 주간·월은 `basket-*-insight` 배치 산출물을 그대로 서빙(새로 안 만듦)
- **체크사항을 두 갈래로 분리** — ⚠️ 프로젝트명 매칭은 폐기했다: **'여수'가 매장 별칭이자 '여수 섬박람회' 프로젝트**라 공간팀 업무 22건이 매장 체크사항으로 딸려 들어왔다
  · 📋 매장 체크 코멘트 = 일일보고 ⑤장비·⑨구매·③송금승인 (읽기 전용, 14건)
  · ✅ 완료처리 대상 = 주간분장 중 매장 담당자(양은정·김준호) 배정분 → 기존 `/api/assign-status` 그대로 (**현재 0건** = 매장 업무가 분장으로 안 올라오고 있다는 사실)
- 외부 I/O 60초 TTL 캐시(`_cached`) — 없으면 진입마다 gws 프로세스 4개

### ⚠️ 실데이터에서 드러난 것 (화면보다 이게 더 큰 문제)
- **매장마감 탭 실데이터 1행뿐** — 리진 2026-07-24. 그 이후 마감보고 **0건**
- **일일보고 탭 매출·지출 칸 최근 보고 전부 공란**
- 굿즈·이벤트매출은 **칸 자체가 없음**, 음료·디저트는 금액이 아니라 잔수·품목 **서술**
- → **매출 세분화 필드 확장을 이번에 넣지 않았다.** 칸을 4개 늘려도 마감보고가 0건이면 빈칸만 4개 는다. **마감보고 제출 복구가 먼저**
- 화면은 감추지 않는다: 조회창 분리(마감 1건 90일 / 주간합계 7일 / 이슈 30일) + 없으면 "최근 90일 마감보고 없음" 명시, 오래된 매출은 "일 매출 (37일 전)"로 라벨

### 배포 (⚠️ 브랜치 지형 기록)
- 라이브 = `origin/arisa-vnext`였고 로컬 워크트리는 11커밋 뒤처져 있었다 → ff-only 동기화 후 작업
- 커밋 `acc5a4d` → `origin/arisa-vnext` push
- ⚠️ **맥미니는 `main` 브랜치이고 origin/main보다 34커밋 앞서 있다**(미push 로컬 작업). 브랜치 병합 대신 **cherry-pick으로 2파일만 얹었다**(`5d4e466`). 두 파일이 내 커밋 base와 바이트 동일이라 충돌 0
- 배포 전 `~/deploy-backups/`에 원본 2개 백업. 재기동 PID 76887 → **94635**

### 검증
| 항목 | 결과 |
|---|---|
| `ast.parse` (노트북 3.9 + 맥미니 3.9) | OK — 40만자 단일파일이라 필수 |
| 셸 내장 JS `node --check` (77KB) | OK |
| `store_access()` 전 직원 12명 | 의도한 4명만 True |
| 테스트 계정 4개 실로그인 (더미 PIN, 격리 데이터) | 3명 200 · 양지혜 **403** (API·HTML 둘 다) |
| Playwright 렌더 (계정별 독립 컨텍스트) | JS에러 0 · 가로 오버플로 0 (1440=1440) · 카드 높이 균일 613px |
| 서브탭 일일→주간→월→일일 | 주간·월 iframe 실제 내용 로드 확인 |
| 팀 일정표 대표·리더·직원 | 전부 최상단 · 18건 · 실제 캘린더 대조 |
| TTL 캐시 | 1.5s → 0.000s |
| **맥미니 실환경 함수 직접 실행** | 캘린더 18건 2.1s · 매장 5곳/체크14 3.8s · insight HTML 2종 OK |
| 배포 후 미로그인 게이트 | `/api/stores`·`/api/calendar`·`/store-insight` 전부 401 |
| **arisa-os.com 공개 도메인 렌더** | 200 · 매장 탭 DOM 존재 · 캘린더 CSS 적재 · JS에러 0 |

### 남은 것
- ⏳ **실계정 로그인 후 화면 육안 확인** — PIN이 없어 대표 계정 실렌더는 미확인. 코드·데이터·게이트는 전부 검증됨
- 후속 과제 A~I는 검토 리포트 §5 참조 (메일 블록·코멘트칸·산출물축·이슈목록·잡무분류·내일할일·일정추가·분장승격·매출필드확장)

## 2026-08-25 — 사업기획팀 2인 7·8월(2개월) 업무보고 분석

- `analysis/business-team-2month-analysis-2026-08-25.html`: 배성원·김도영 7/1~8/16 시트 원본(핵심 88→중복제거 86·서브 363→357·메타 44) 기반. 7/22 3주 분석의 2개월 확장판
- **지표**: 배 보고 16/33일(48%)·핵심 36·서브 139·산출물 19%(7월 21%→8월 12% 악화)·**완료 18건 중 15건(83%) 산출물 미기재** / 김 보고 24/33일(73%)·핵심 50·서브 218·산출물 60% 유지·완료 미기재 10/31(32%)
- **반복=지연 진단**: 핵심+서브 443건을 18개 클러스터로 묶어 W27~W33 체류 계산. 단계전진형/재작업·대기형 수동 분류 → **🔴 5개(TIPS·선금증빙·신규개발MOU·섬별콘텐츠마스터·연구전담부서), 그중 4개가 배성원 담당이며 전부 "상대 회신·대표 결정 대기"에 산출물 0~33%**
- **정상 종결 확인**: 인허가(단계 매주 전진)·패널 38매·ADICO 물류·RCMS(7/23 1800만원)·금융(고위드·대출 7/16 해소)·공간렌더(8/5 오픈). 단 공간/렌더 47건 중 38건이 W30~31 2주 집중 = 확정 지연발 재작업 비용
- **카테고리 분리**: A 프로젝트형 9종(P1 여수1차 141건 ~ P9 내부체계) / B 상시형 74건(세무14·정부정산34·인증12·금융6·인사8) — B는 반복이 정상이므로 지연 판정 제외
- **완료 산출일정 8건 제시**(선금증빙 8/26 → 연구전담·월드모드 8/27 → MOU·마스터플랜 8/29 → 콘텐츠마스터·P1정산 9/1 → Pre-A 9/5). 목표는 성사가 아니라 **체결/보류/폐기 중 하나로 상태 확정**
- ⚠️ **한계 2가지**: ①시트 마지막 보고 8/16 → **8/17~8/25 영업일 7일 공백**, 모든 진행중 판정이 8/16 기준 ②팀원 실제 파일은 개인PC·공유폴더 보관으로 **실물 대조 불가**(워크스페이스·구글드라이브 검색 0건) — "미기재"는 산출물란 공란이지 물건 부재가 아님
- 마스터플랜(배)은 7/28 이후 8월 언급 0건 → 중단 여부 확인 필요
- `analysis/business-team-weekly-ledger-2026-08-25.html`: **주간 산출물 레저 W27~W35(9주)** — 좌측 보고실적(주평균 대비 증감)·보고내용, 우측 **보고 기반 예상 산출물(✅기재분 37건 / ◻︎추정분)** 병렬 배치. 6월 데이터 0건 확인(2인 모두 7/1 보고 시작)으로 앞 확장 불가 → 실보고 7주 + 미보고 2주(W34·W35) 구성
- 주평균 기준선: 팀 핵심 12.3·서브 51.0 / 배 핵심 5.1·서브 19.9·보고 2.3일·주당✅ 1.0 / 김 핵심 7.1·서브 31.1·보고 3.4일·주당✅ 4.3
- 주별 합계(핵심/서브/✅): W27 10·45·6 → W28 14·48·8 → W29 14·55·4 → **W30 16·64·5(정점)** → W31 14·61·7 → **W32 8·59·2(오픈주인데 배성원 ✅ 0건 — 9주 중 유일)** → W33 10·25·5
- **발견**: ①바쁜 주일수록 기록이 끊긴다(여수 오픈 W32에 특약매입11건·통합운영계획·안전서류3종이 실제 나왔는데 ✅ 최저) ②W30~31 김도영 서브 101건=전체 46%, 상당수가 DECK v2→v5·렌더 재교체 등 **확정 지연발 재작업** ③배성원 ◻︎는 계약서·납부확인서·접수증 등 원래 실물이 남는 문서 → **기재 규칙이 업무유형에 안 맞는 것**
- 개선안: 배=파일명 대신 "증거 한 줄"(계약서명/접수번호/납부일·금액/결정사항) 허용 · 김=버전 올라가면 **왜 올라갔는지(누구 확정이 늦었는지)** 병기 · 공통=오픈·마감 주간은 주말에 몰아 1회 기재
- `analysis/business-team-weekly-ledger-2026-08-25.xlsx`: 위 주간 표의 엑셀본(14컬럼 × W27~W35). 주차·기간·사건 셀 병합, 담당별 2행, 필터·틀고정(D7), 지표는 AVERAGEIF/SUM 수식(하드코딩 금지) → recalc 검증 **수식 57개·오류 0**. 하단 요약행 계산값 원본 일치 확인(배 평균 2.29일·5.14·19.86·✅1.0 / 김 3.43일·7.14·31.14·✅4.29 / 합계 보고40일·핵심86·서브357·✅37)
- 개인 시트 분리 추가 → **3시트(주간레저·배성원·김도영)**, 개인 시트는 담당 컬럼 제거 13컬럼·주차당 1행, 평균은 자기 7주 AVERAGE 기준. recalc 재검증 **수식 119개·오류 0**. 개인 합계 일치(배 보고16일·핵심36·서브139·✅7·기재율19% / 김 24일·50·218·30·60%)
- **간트 시트 2종 추가 → 최종 5시트**(`배성원-간트`·`김도영-간트`). 프로젝트 항목별 × W27~W35 셀에 그 주 보고건수 표시 + 상태별 색농도 4단계(🟢완료/🟡진행/🔴지연), 시작~종료 사이 무보고 주는 회색, W34·W35는 판정불가 회색열. 컬럼: 항목·분류(A프로젝트/B상시)·상태·총건수·시작·종료·지속 + 9주 + 확정산출물·잔여액션·완료목표일. 틀고정 H7. recalc **수식 135개·오류 0**
- 간트 항목 수: 배 14개(🟢5·🟡3·🔴6) / 김 12개(🟢6·🟡4·🔴2 — 콘텐츠·선금 2건이 지연). 합 🟢11·🟡7·🔴8. ⚠️ 키워드 매칭이라 한 업무가 복수 항목에 중복 계상 → 항목 합계(배 142·김 209)는 총보고건수(배 175·김 268)와 불일치, 라벨에 "중복 계상 포함" 명시
- 간트에서 새로 드러난 것: 김도영 **패널/그래픽은 W30~31 2주에 24건 집중**(단기 폭발형), **계약은 W31~32 2주 14건**(집중 처리형) — 반면 배성원은 🔴 6건이 전부 **6~7주 장기 체류**(CJ×KBO만 정상 감쇠 8→7→2→1→1→·→2). 즉 김=단기집중·배=장기체류로 부하 패턴 자체가 다름 → 처방도 다름(김=확정 앞당겨 폭발 분산 / 배=6주 끄는 걸 2주에 닫기)
- **판정 기준 정리**: 체류 길이가 아니라 **전진 여부**가 기준 — 김도영 인허가 5주(3-1-3-5-1)는 매주 단계 전진이라 🟢, 배성원 연구전담부서 5주는 같은 자리라 🔴
- `analysis/business-team-weekly-ledger-2026-08-25.html`에 **3장 간트 섹션 추가**(장 번호 3→4→5 재조정). CSS grid 9칸 트랙, 상태별 색농도 4단계, 구간 내 무보고=회색, W34·W35=빗금. 구조검증: 26행×9셀 전건 일치·CSS 미정의 클래스 0·셀합=표기건수 일치
- **2장 주간 레저를 3탭(통합/배성원/김도영)으로 분리** — `input[type=radio]` + `:checked ~` 형제선택자만 사용한 **순수 CSS 탭**(JS 0줄, 에러 리스크 없음). 개인 탭은 담당 컬럼 제거 9컬럼·주차당 1행, 증감은 **본인 7주 평균** 기준, 하단 합계행 포함
- ⚠️ **MCP playwright가 다른 세션에 점유**되어 사용 불가 → `playwright.sync_api`로 직접 headless 실행해 검증. 결과: 탭 전환 3종 전부 정상(클릭 시 해당 pane만 display:block)·**JS 에러 0건**·가로 오버플로 없음(doc 1440 = win 1440)·배성원 탭 11행 렌더. 개인 탭 행합계=합계행 표기 일치(배 16/36/139/7 · 김 24/50/218/30)

## 2026-08-25 — 최원석 본인 업무 9주 산출물 구조 분석 (사업팀과 동일 구조)

- `analysis/wonseok-worklog-2026-08-25.html`: W27~W35(9주). **소스가 시트가 아니라 워크스페이스 git 커밋** — 최원석 시트 기록이 7~8월 핵심 4건뿐이라 분석 불가, "Claude와 함께 만든 실제 작업"으로 대체
- 규모: 커밋 205건(214 중 server-mini 8 + 7/5 일괄스냅샷 1 제외) · 변경파일 1,529 · 신규 문서산출물 255건 · **실존 231건(91%)**
- **사업팀 대비 핵심**: 산출물 확인율 최원석 91%(실물검증) vs 배성원 19%·김도영 60%(기재기준). 단 이 차이는 성실성이 아니라 **매체 차이** — git은 증거가 자동으로 남고, 계약·협상·행정은 파일로 안 남음. 결론은 "누가 더 일했나"가 아니라 "증거가 자동으로 남는 구조인가"
- 주차별: W27 7/89/3 → W28 33/116/14 → W29 52/101/10 → **W30 58/628/108(전지표 최대)** → W31 40/304/34 → W32 10/40/**0(코드중심)** → W33 1/3/1(최저) → W34 4/248/85(봉은사 집중) → W35 0
- 간트 15항목(🟢종결 8·🟡진행 3·⚪정지 4+1). **⚪정지 = "중단"이 아니라 "git으로 판정 불가"** — AX 사전진단·US 플레이북·R-Europe(각 5주)·데일리기록(4주). 배성원 🔴 6건과 같은 성격의 확인 목록
- **조직 차원 발견**: ①**W30이 3인 동시 정점**(최원석 커밋 58·김도영 서브 51·배성원 마스터플랜 착수) ②**W32~33에 3인 동시 붕괴**(여수 1차 오픈 8/5 주간) — 현장이 열리면 기록이 멈추는 패턴이 조직 공통 ③ARISA 3종이 커밋 155/205=76%로 단일 집중도 매우 높음 ④8월 축이 ARISA→봉은사로 이동
- ⚠️ **버그 정정**: 최초 집계에서 한글 파일명이 git octal escape로 나와 `os.path.exists` 실패 → 실존율 27%로 오산출. `core.quotepath=false` 재추출로 **91%** 확정. 사용자 보고 전에 교차확인으로 발견
- 한계 명시: git 커밋분만 포함(대화·검토·의사결정·맥미니 직접배포·외부시스템·arisa2 별도레포 제외) → **실제 작업량의 하한선**. .png 제외, 코드·md 제외로 보수적 집계
- 렌더 검증: 간트 15행×9셀·레저 10행·가로 오버플로 0·JS 에러 0

### v2 전면 재작성 (같은 날, 사용자 지적 반영)

- ⚠️ **v1이 절반 이상 누락**. 사용자가 "현대자동차 개발검토 빠졌어"·"SK제안서도 빠졌는데?? 빠진게 많아" 지적 → 원인 2가지: ①**커밋을 대표 스코프 1곳에만 귀속**시켜 여러 프로젝트를 건드린 커밋이 묻힘 ②**git 미추적 파일 827건(36%) 통째 누락**
- 미추적으로 빠졌던 것: 성수 공공팝업 93파일·MaaS 47·**현대차 X-Discovery 33(문서 25)**·R_define 21·롯데 8·광희바자르 8·**SK 하이닉스 7**·롱블랙 4·병원 정부과제 4·서울디자인어워드 4 — 전부 커밋 0건
- **재집계 방식 전환**: git 커밋의 모든 파일을 **경로 기준으로 전 프로젝트에 계상**(대표 1곳 귀속 폐기) + git에 없는 파일은 **mtime 보완**. 결과 **프로젝트 15→39개, 파일 1,529→2,281건**
- **소스 3종으로 확장**: 파일 2,281(문서 524) + **캘린더 미팅 60건**(본인캘린더 59+PROJECT RENT 102 → 생일·연차·송금 제외, 본인 관련만) + **보낸메일 195건**(from:me 229 → 기간필터)
- **핵심 발견 — v1의 "8월에 멈췄다"는 착시**: 커밋은 W32~33에 10→1로 붕괴하지만 같은 주 **메일 19·23건, 미팅 9·4건**. 현대차 X-Discovery 착수·특허 P260007 최종검토·홈페이지 문의 11건 회신이 그 구간에 있었음 → **일이 준 게 아니라 만드는 일→대응·협의하는 일로 종류가 바뀐 것**
- **자기 함정 확인**: 배성원 "완료인데 산출물 없음 15건"과 같은 구조에 내 분석도 빠져 있었음. **아랑재는 파일 0건인데 미팅 3·메일 12** → 파일만 보면 존재하지 않는 업무
- 간트 2종 분리: **산출물형 29건**(파일 중심) / **소통형 9건**(아랑재·홈페이지문의 32건·아파트너투자·현대백화점·금융정산·강의미디어·세스크멘슬·법률행정·내부회의) — 색농도는 프로젝트 자체 최대치 대비(규모 편차 보정)
- 기타 발견: ①W30(넓은 과부하)·W34(봉은사 348파일 깊은 집중) 두 봉우리 ②미팅은 8월에 오히려 증가(7월 평균 5.6→8월 8, W34 14건 최다) ③**홈페이지 문의 32건이 상시 부하**로 매주 5~8건 ④봉은사 8/18 최종보고서 송부 후 8/20~23에 348파일 재작업(납품 후 변경)
- 검증: 간트 38행×9셀·레저 9행 합계=헤더 표기 완전일치(2,281/524/60/195)·가로 오버플로 0·JS 에러 0
...
### 최원석 워크로그 엑셀 (2026-08-25)

`analysis/wonseok-worklog-2026-08-25.xlsx` — HTML v2와 같은 데이터, 5시트.

**HTML과 다른 점**: HTML 간트는 상위 29개만 실었으나 **엑셀은 분류된 44개 전부**를 넣어 주간 합계가 정확히 맞는다. 주차별 파일 합(101·174·96·589·670·79·94·438·40)이 프로젝트별 합과 일치 — 미분류 잔여 0.

| 시트 | 내용 |
|---|---|
| 주간레저 | W27~W35 × 파일/문서/문서율/미팅/메일/활동합계 + 주제·업무·미팅·산출물 서술 |
| 프로젝트-산출물형 | 44개 × 9주 간트(행 최대값 기준 4단계 채색, 중간 공백 회색) |
| 프로젝트-소통형 | 파일 0건 카테고리 10종 × 9주 접점(미팅+메일) |
| 미팅60 / 보낸메일195 | 원본 — 주차·날짜·제목·상대 |

**검증 내장**: 주간레저 18행 "검증"이 프로젝트별 시트 총계를 역참조하는 SUM. 파일 2,281 / 문서 524 / 미팅 60 / 메일 195 — 합계행과 완전 일치. recalc 113수식 0에러.

**HTML에 없던 15종 추가 수록**: 워크스페이스 루트 78 · R-Tech 스크립트 19 · 시스템 9 · 문서 6 · 지식 아카이브 6 · R-Tech 4 · Basket 3 · Team Ops 3 · 인건비 시뮬 3 · Second Brain 3 · 세스크멘슬 SNS 2 · 매출목표 모델 2 · 성수팝업 방향 2 등 (합 147파일).

### v3 — 계약·정관·제안·레포트를 산출물로 재분류 (2026-08-25)

**지적**: "정관, 계약서 검토 및 수정본 만드는 것도, 제안서나 레포트 만드는 것도 모두 산출물로 잡아줘"

**원인**: 문서 524건을 한 덩어리로만 셌고, 계약·법무 업무를 "파일이 안 남는 소통형"으로 분류했다. 실제로는 현대차 계약 패키지 12건·성수 법무 23건이 프로젝트 폴더 깊숙이 있었다.

**조치 3가지**
1. **문서 8유형 분해** — 계약·법무 46 · 제안·기획 23 · 보고·분석 195 · 가이드 11 = **핵심 산출물 275건(52.5%)**. 기타 제작물 91 포함 시 366. 나머지 158(자동생성 배치 59·수집 원자료 67·백업 32)은 "쌓인 것"이라 분리.
2. **계약·제안 원장 신설** — Gmail 전체(보낸+받은) 키워드 306건 → 실무 스레드 16건 / 126메일. **8건은 워크스페이스에 파일 없음**.
3. **접점형에 성격 부여** — 계약·제안 / 영업 / 행정 / 내부. "법률·행정"은 산출물 있음으로 정정.

**메일에서만 발견된 업무 (v1·v2에 아예 없음)**
CJ ENM KBO 팝업 제안 13메일 · 아랑재 계약서 검토 13 · Project M 325 NDA(일우건설·케이핀자산운용) 8 · 정관·등기부·구주양수도 8 · 크래프톤 Culture Ground 자문 6 · 바스켓 임대차 만료 · OF갤러리 계약서 · 롯데시네마 DUNE3 견적

**버그 정정**: macOS 경로 일부가 NFD(자모 분해)라 한글 키워드 매칭 실패 → 전 경로 NFC 정규화 후 계약·법무 39→46. **파일 수 중복 계상은 0건** 확인, 총계 2,281 유지.

**정정**: "계약·법무가 8월에 몰렸다"는 부정확 — 실제 W27~29는 0건, W30 시작, **W31(16)·W33(15) 두 정점**. 성수 법무는 24가 아니라 23건.

**산출물**: `wonseok-worklog-2026-08-25.html` v3(8섹션·간트 3종 29/9/16) + `.xlsx` 8시트(문서유형·계약제안원장·계약제안문서목록 추가, 180수식 0에러)

**남은 것**: 미분류 문서메일 126건 — 개별 확인 시 계약·제안 스레드가 더 늘어난다.

### v3 자체검토 — 7건 발견, 5건 수정 (2026-08-25)

**발견**
1. **HTML·엑셀 불일치** 🔴 — 접점형이 HTML 9건(104) vs 엑셀 10건(185). HTML에 기타 소통 82·여수 1 누락, 세스크멘슬은 HTML에만. → **10건 185로 통일**
2. **색 농도 오독** 🔴 — §5·§6은 같은 단위(메일·미팅 건수)인데 행별 정규화라 "메일 1건"과 "10건"이 똑같이 진했다. → **표 전체 기준으로 변경**
3. **보고·분석 위양성** — MaaS를 "보고서 프로젝트"로 통째 편입해 단가표·상품가이드 14건이 레포트로 잡혔다. → 195→**180**
4. **계약·법무 위양성** — `_test.docx`·`_ref.docx`·`~$` 등 변환 테스트 더미 5건. → 46→**43**, '작업 부산물' 유형 신설
5. **제안·기획 위양성** — `strategy-proposal-report-guide.html`은 제안서가 아니라 가이드. → 23→**22**
6. **포맷 변형 중복**(미수정·공지) — 같은 제안서의 html·pdf·pptx가 3건. 고유 제안서는 22건이 아니라 **약 12종**
7. **수집·원자료 mtime 왜곡**(미수정·공지) — 67건 중 65건이 W34에 몰림. 실제로 그 주에 모은 게 아니라 마지막 수정 시점만 남은 것

**최종 수치**: 계약·법무 43 · 제안·기획 22 · 보고·분석 180 · 가이드 14 = **핵심 산출물 259건(49.4%)** / 기타 제작물 102 포함 시 직접 만든 것 361 / 나머지 163(자동생성 59·수집 67·백업 32·부산물 5)

**검증**: HTML 8섹션·간트 29/10/16·JS에러 0·가로overflow 없음 · 엑셀 182수식 0에러·검증행 일치(2281/524/60/195)

### v4 — 법무 범위 전면 확장 (2026-08-25)

**지적**: "체크하고 계약서검토 등 법무적인 검토사항들도 모두 업무 및 산출물에 포함해줘"

**근본 원인**: 문서 집계 확장자가 `html/xlsx/pdf/pptx/docx`로 고정돼 있어 **`.md`·`.txt`·`.zip` 법무 산출물이 통째로 제외**됐다. 성수 법무 대응의 핵심 문서가 전부 `.md`였다.

**추가된 것 34건**
- 성수 공공팝업 32 — 1차의견서-본문 · 00-사건개요및검토요청 · 01-증거목록 · 붙임1 사업경과 타임라인 · 붙임3 귀구청 요청안내 · 카톡 3인 핵심발췌 · 공식문서 2건 분석 · 변호사전달_20260815.zip · 변호사문의 2건 + 증거 원자료 22(카톡원본 3·메일원본 15·녹취록 2·공문 전사)
- 현대차 2 — 회신문초안 260807·260811

**지식재산 6건 재분류** (기타 작업물 → 계약·법무): CFD-ARISA-특허전략 v1·요약, 특허명세 1단계 초안(html·docx), 욕구모델-특허도면(html·docx)

**최종 수치**: 문서 **524 → 558**, 계약·법무 **43 → 61**
| 유형 | 건수 |
|---|---:|
| 계약·법무 | 61 (10.9%) |
| 제안·기획 | 22 |
...
**계약·법무 61 내역**: 성수 30 · 현대차 16 · HR 6 · ARISA 특허 4 · 봉은사 2 · 인건비 2 · 개인 1
**주차별**: W27~29 = 0 → W30(9)·W31(15)·W32(9)·**W33(22 최대)**·W34(6). 파일 총량은 8월에 줄었으나 **법무 문서는 8월에 최대**.

**새 발견 — 법무는 2건의 실제 분쟁이 끌고 있다**
1. 성동구청 공유재산 사용허가 종료 — 증거수집(카톡 3인·녹취 2·메일 15 재구성) → 증거목록 → 1차 의견서 → 회신공문 발송(8/19) → 변호사 문의(8/18). **3주 완결 법무 프로세스**
2. 현대차 기술용역계약 — 검토의견서 2회(8/7·8/11) + 별첨 3종 + 회신공문 2회 = 조항 협상

**검증**: HTML 8섹션·간트 29/10/16·JS에러 0 · 엑셀 182수식 0에러 · 검증행 일치(2281/558/60/195) · 문서목록 시트 83건(법무61+제안22)

### v5 — "어제오늘 산출물이 왜 없냐" 전수 재조사 (2026-08-25)

**지적**: 세스크멘슬 브랜딩 제안 / 현대차 개발범위 확정·계약서 리뷰 / 정관 등 RXR 서류 / T팩토리 제안서 / LA·K글로벌 유니원 계약서 검토 / 공간디자인 자료 / 시공마스터플랜 — 전부 안 보임

**사각지대 3종 발견**
1. **스캔 시각** — 최초 스캔이 오늘(8/25) **11:24**. 오후 작업 전량 누락. W35 파일 40 → **46**(+job tmp 70), 메일 7 → **11**
2. **Claude 작업폴더** — `~/.claude/jobs/*/tmp/`의 **2,341파일·문서 441건이 0건 집계**. HR 포털 이관 1,380 · KCCLA K-존 464 · 현대차 RXR 정성분석 219 · 봉은사 PPTX 47. **KCCLA는 워크스페이스에 최종본 5개뿐인데 작업폴더에 464개**
3. **워크스페이스 밖** — 바탕화면(`LA한국문화원_2026_방문객수.xlsx` 8/24) · 다운로드(세스크멘슬 제안서 v6) · 맥미니 원격

**지목 7건 판정**: ✅잡힘 1(현대차 회의록 수정본+회신메일) / △부분 2(세스크멘슬=미팅만, T팩토리=메일만) / ❌누락 2(RXR ProjectD·E 70파일 job tmp, 유니원 자료 바탕화면+K존 464) / ❓미확인 2(공간디자인, 시공마스터플랜 — 파일 0건)

**자체 버그 정정**: 재스캔 1차에서 `.omc/` 런타임 상태파일 61건을 작업물로 셈 → W35를 107로 과대보고할 뻔했다. `(^|/)\.omc/`로 수정해 **46**으로 확정.

**최종**: 파일 2,287 · 문서 573 · 미팅 60 · 메일 199 · 핵심 산출물 279
**산출물**: HTML 9섹션(§7 사각지대 신설, 간트 4종 29/10/16/9) + 엑셀 9시트(사각지대-job폴더 추가, 202수식 0에러, 검증행 완전 일치)

**남은 것**: 공간디자인 자료·시공마스터플랜 소재 확인 필요. job tmp 2,341은 본 집계에 미합산(중간 산출물) — 별도 축으로만 공개.
