...
## 2026-08-11 — 분장 액션 패널 실서비스 반영 완료 (end-to-end 검증)

### 배포
- `b4345c4` → `origin/arisa-vnext` push, 대시보드 재기동(PID 44753 → **76887**), arisa-os.com 200
- ⚠️ **맥미니에서는 이 레포에 push할 수 없다** — `~/do-better-workspace`의 origin이 HTTPS(`ai--brain.git`)인데
  맥미니에 자격증명이 없고(`could not read Username`), SSH URL로 바꿔도 그 레포엔 키 권한이 없다
  (`hr-workspace`는 origin이 `do-better-workspace.git` SSH라 통한 것 — **두 워크스페이스가 다른 원격**).
  **우회**: 노트북에서 맥미니를 임시 리모트로 붙여 커밋만 fetch → `git push origin macmini-tmp/main:arisa-vnext` → 리모트 제거.
  로컬 워킹트리는 건드리지 않는다

### 실서비스 검증 (대표 계정, `/my-brief`)
- 패널 렌더: `✅ 내 열린 분장 7건 · 승인 대기 1건`, 항목 8 · 버튼 20 · **핸들러 바인딩 20/20**, 콘솔 에러 0
  (콘솔은 한 번 비우고 재로드해 확인 — 처음 본 401·Babel은 직전 HR 포털 페이지의 잔여 로그였다)

### 진행률 → 시트 반영 end-to-end (드디어 끝단 확인)
| 경로 | 결과 |
|---|---|
| 변경 전 API | `row 32 · progress "" · progress_pct 50` (P열 비어 있음, 50%는 상태 파생 자동값) |
| 조작 | 브리프 화면 셀렉트에서 60% 선택 (Playwright selectOption = 실제 change 이벤트) |
| 화면 | `진행중 · **60%**` (전엔 % 표시 자체가 없었다) |
| 대시보드 API | `progress "60" · progress_pct 60` |
| **시트 직접 읽기(gws, 대시보드 미경유)** | `주간분장!P32 = "60"` |

마지막 줄이 핵심 — 대시보드를 믿지 않고 `gws sheets +read`로 따로 읽어 대조했다.
32행 = `최원석 / 예산안 및 work scope 확정 / 2026-07-30 / 진행중 / … / 기획 / 60`.

### 잔여 1건 → ✅ 같은 날 원복
- 테스트로 넣은 60%(LA한국문화원 예산안, `주간분장!P32`)를 **gws로 clear**해 원상 복구.
  화면도 `진행률 50% · 자동`으로 복귀 확인
- **UI로는 진행률을 지울 수 없다** — 빈 옵션은 `if(!v) return`이라 저장 동작이 없다.
  되돌리려면 시트 셀을 직접 비워야 한다:
  `gws sheets spreadsheets values clear --params '{"spreadsheetId":"…","range":"주간분장!P32"}'`
  (**`--dry-run`으로 대상 URL부터 확인할 것** — 범위를 잘못 주면 데이터가 날아간다)

### 메모
- browse의 `select`는 같은 클래스가 여러 개면 타임아웃난다. **JS로 임시 id를 붙이고 그 id로 selectOption** 하면
  실제 사용자 조작 경로를 유지하면서 지정할 수 있다

## 2026-08-09 — 직원 화면 실측 + 분장 액션 버튼 (✅ 08-11 반영 완료)
...
### 남은 것 → ✅ 08-11 전부 해소 (아래 08-11 섹션 참조)
1. ~~서비스 재기동~~ → PID 44753 → 76887
2. ~~커밋·push~~ → `b4345c4` → `origin/arisa-vnext`
3. ~~버튼 실동작 확인~~ → 진행률 60% 시트 반영까지 확인
...
### 봇 분할 — 아리사(분장·보고) / 알군(조회·질의) (2026-08-09)
설계서 `bot-split-design.md`. 커밋 `c8a6365`→`d153164` (arisa-vnext).

**결정 전환**: 오전엔 "한 봇 + 보고 무상태화"였으나, 대표 판단으로 분할. 그런데 **그 경계가 정확히 "대화 상태 있음/없음" 경계였다** — 분장→보고→완료는 여러 턴 흐름(상태 필요), 규정·조회는 물으면 끝(상태 불필요). 8/3 사고(질문이 업무 내용으로 저장·세션 6일 갇힘)의 뿌리가 "상태가 조회를 잡아먹는다"였고, 갈라두면 조회 쪽엔 갇힐 상태가 존재하지 않는다.

**아리사** `@Dailywork_report_bot` — 이름 변경(구 '업무보고 도우미'), 메뉴 등록(구 `getMyCommands`=[] → 직원이 `/report` 존재조차 몰랐음). 분장·보고·완료환류·마감보고.
**알군** `@R_rent_bot` — 신규, PID 49809 가동. **ConversationHandler 0개**(계약). 규정·프로젝트·회의록·브리프. mem0 미사용(맥미니 전역 lock 회피).
**핸드오프** — `INTENT_BOT` 단일 출처. 잘못 들어오면 링크로 넘긴다(막지 않음).

**사내 규정 QA** (`shared/policy_docs.py` + `shared/gws.py` Drive 확장)
- Drive 규정 6종 `.docx`(각 4~5천 토큰) → **임베딩·벡터DB 불필요.** 청킹하면 "제N조 단서" 맥락이 끊긴다
- 🔴 **계약: 원문 인용 못 하면 답하지 않는다.** LLM이 found=true라 해도 인용문이 원문에 없으면 기각 — 조작 인용 주입으로 기각 실증
- 실측 함정: ①`.docx` 표를 셀 단위로 뽑으면 항목↔값 짝이 사라져 LLM이 오답("연차 1일 이하=3일 전") → 행 단위 ` | ` 결합 ②`drive files download`는 API Internal error, **`files get`+`alt=media`가 동작 경로** ③gws는 cwd 밖 `--output` 거부
- 검증: 동기화 6/6 · 문서선택 6/6 · 답변 4/4(없는 것 2건 거절 포함) · 라우터 22/22

**`/assign` 오염 차단** — 시트 156행 중 **텔레그램 등록분 0건**이라 버그가 드러나지 않고 있었다.
- ①`@지혜님`→LLM `지혜`→명부 미매칭→**담당자 빈 채 저장**(아무도 안 보는 행) ②LLM이 `team="BM"` 생성, 구 코드는 team이 비었을 때만 명부를 봐서 오염 그대로 저장
- `shared/employee.py:resolve_name()` — 존칭·성 생략 흡수, **애매하면 안 고름**(동명이인 시 후보 반환). 엉뚱한 사람에게 업무가 붙는 게 못 찾는 것보다 나쁘다
- 교정 로직이 3곳 복제 → `_fix_assignee_team()` 하나로

**⚠️ 내 오판 정정**: vNext 작업을 "미push"로 판단했으나 **틀렸다.** `origin/arisa-vnext`에 이미 push돼 있었고 맥미니 로컬 `main`이 그 브랜치를 추적한다 = **맥미니는 vNext로 배포된다.** `git am`이 아니라 정식 머지가 맞았다.

**별건 발견**: `dashboard-server.py:267`이 없어진 모델(`claude-sonnet-4-20250514`)을 부른다 → Anthropic 매번 실패, OpenAI 폴백으로만 가동 중. 현행 `claude-sonnet-5`는 `temperature`도 거부.

**남은 것**: 아리사에서 조회 기능 제거(**알군 실사용 확인 후**) · `/assign` 대표 전용 유지 여부 · 대시보드 모델명 · 연차 잔여 API(🔴 기존 서비스 토큰 재사용 시 `_scope_rows_to_self`가 전 직원 반환) · @brocallmebot 토큰 로테이션
