...
- 직원 실보고 13건 자동 채점 (20~30점대 — 교육 전 베이스라인), 질문 루프 asked=3, 시트 N/O열 기록 전부 정상
- `leader-share ok` 로그로 팀 리더 공유 실작동 확인 (양지혜→윤혜정 등) — 7/4 제보 해결
- 7/7 저녁 gws 인증 갭(credentials.enc 삭제 재발 — 원인: 백엔드 불일치 시 gws가 자체 삭제, 재인증 완료) 중에도 보고 유실 0건 — 하드닝 큐→백필 검증됨
- 다음: 교육 세션 → N열 점수 추이 관찰 → v1.2(주간 대시보드 점수 축)
