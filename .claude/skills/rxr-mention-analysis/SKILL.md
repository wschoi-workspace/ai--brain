...
> **⚠️ 측정 함정 — 공간·컨셉별 SOV는 문자열 매칭 금지 (2026-09-09 청담나인 사고)**
>
> 브랜드 **주소·층·번지 표기가 키워드와 겹치면** 단순 OR 매칭이 구조적으로 부풀려진다.
> 청담나인(삼성로 634 **B1, B2, 10F**)에서 `루프탑|10F|10층` 매칭은 22.5%를 냈지만
> 실제(토픽 판정)는 **2.8%** 였다 — 블로거가 주소를 그대로 복사하기 때문이며, 40건 중 12건이 주소 표기뿐이었다.
> "인지가 커지는 중"이라는 **정반대 결론**이 나올 뻔했다.
>
> **적용 규칙**
> 1. 측정 전 브랜드 주소·층·전화번호가 키워드와 겹치는지 확인
> 2. 겹치면 ①주소 패턴을 정규식으로 제거 → ②**관련어 2개 이상**일 때만 카운트
> 3. 수치가 예상보다 높으면 **원문 샘플을 열어 왜 잡혔는지 확인**한 뒤 결론
> 4. 리포트에 측정 방식(토픽 판정 적용 여부)을 명기
>
> 고유어(재즈·와인 등)는 단순 매칭도 안전. 위험한 것은 **층·번지·숫자 표기**다.
...
**R Design Guide 준수**: SUIT Variable, 기본 보라 #6C5CE7
