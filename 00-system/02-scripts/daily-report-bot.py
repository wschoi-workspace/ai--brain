...
반드시 JSON만 출력:
{"outcomes": ["", ""], "decision_needed": "", "support_needed": "",
...
# 백필 dedup용: 탭별 중복 판정 컬럼(날짜·이름·내용 식별자)
_DEDUP_COLS = {"핵심업무!A1": [0, 1, 5], "서브업무!A1": [0, 1, 4], "메타!A1": [0, 1]}
...
def _build_sheet_rows(d: dict) -> list[tuple[str, list]]:
    ...
    for st in d.get("sub_tasks", []):
        rows.append(("서브업무!A1", [d["date"], d["name"], d["team"],
                     st.get("category", ""), st.get("task", ""), st.get("status", "")]))
    attach_cell = "\n".join(
...
def save_to_sheet(report_data: dict) -> dict:
    ...
    try:
        ...
        return {"total": 0, "failed": [], "err": "transient"}
...
async def finalize_and_send(
    ...
    if not sv["failed"]:
        ...
        # 정직한 안내: 유실 아님(로컬 큐 보관·자동 백필) — 직원이 재전송할 필요 없음
        status_lines.append(
            "💾 시트 저장이 지연되고 있어요 — 보고 내용은 안전하게 보관했고 자동으로 재시도됩니다.\n"
            "   (다시 보내실 필요 없어요!)")
        cause = ("gws 인증 장애 — 서버에서 `gws auth login --full` 필요"
                 if sv["err"] == "auth" else "일시 오류")
        send_to_manager(
            f"🚨 시트 저장 실패 · {report.get('name','?')} ({len(sv['failed'])}/{sv['total']}행)\n"
            f"원인: {cause}\n"
            f"→ 로컬 큐 보관됨, 백필 배치가 자동 재시도합니다.")
    status_lines.append("\n수고하셨어요! 내일도 화이팅 💪")
