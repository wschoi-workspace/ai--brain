...
## 2026-07-22 (2차) — 구 URL 301 리다이렉트
- 검색 색인에 구 사이트 하위 페이지(/about, /28 등)가 남아 404 → `_redirects`(명시 경로 301) + `404.html`(noindex + 메인 이동 폴백) 배포
- ⚠️ catch-all(`/:page`) 금지 — sitemap.xml/robots.txt까지 리다이렉트돼 깨짐. 알려진 경로만 명시
- 검증: /about·/28·/archive/* → 301, 미지정 경로 → 404 폴백, sitemap/robots/메인 → 200
- 검색 반영: 도메인은 노출 중이나 제목·설명은 구 사이트 캐시 — 재크롤링 2~7일 소요 예상
