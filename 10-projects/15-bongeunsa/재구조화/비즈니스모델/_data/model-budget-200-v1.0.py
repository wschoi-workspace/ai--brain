# -*- coding: utf-8 -*-
"""총투자 200억 예산 구조 v1.0 — 보수예산 / 상업화예산 분리
...
def structure():
    ...
    return dict(repair=a, commercial=b, design=d, pre=PREOPEN,
                sub=sub, contingency=cont, total=TARGET)
...
# ═══════════════════════════════════════════════════════════════
AFTER_TAX = {          # 5년차 세후이익 (model-final 기준)
 "국비 0":    26.8,
 "국비 43.4": 30.0,
 "국비 72.8": 32.2,
}
...
def incremental_return(subsidy_label, lease_repair, subsidy=0.0, loan=0.0):
    inc_inv = incremental(lease_repair) - subsidy - loan
...
def cash_need(subsidy=0.0, loan=0.0):
    s = structure()
...
if __name__ == "__main__":
    W=104
    print("═"*W); print("총투자 200억 예산 구조 v1.0 — 보수 / 상업화 분리"); print("═"*W)
    s = structure()
    ...
    print(f"\n  예비비 {s['contingency']:.1f}억 = 총투자의 {s['contingency']/TARGET*100:.1f}%")
    print(f"  ※ 법난기념관 139% 증액 선례 감안 시 적정 수준(통상 5~10%)")
